...
本仓库基于 [lh-hg/rtt_t2](https://github.com/lh-hg/rtt_t2) fork，并结合当前项目的实际使用场景做了持续整理和定制。它仍然定位为一个免费、易用的 Windows 调试工具，主要面向 RTT / 串口日志查看、数据发送和波形观察等典型嵌入式调试需求。

当前仓库在保留上游核心能力的基础上，重点维护以下内容：

* 面向当前工程的目录结构、构建入口和开发文档整理。

* 配置文件与日志目录管理收敛，降低直接运行和打包后的环境差异。

* RTT 诊断、历史发送记录、文本搜索等使用细节优化。

* 针对现有使用流程的小范围功能调整与兼容性修正。
...
如果你关注当前 fork 版本的源码组织和维护方式，建议优先阅读仓库内的开发文档，而不是直接参考上游说明。
...
如果你是从上游项目迁移过来的，可以把这个仓库理解为“面向当前团队使用习惯整理过的维护版”。文档、构建入口和部分交互细节已经按当前仓库结构重新梳理；功能主体仍与上游 RTT_T2 保持一致，后续新增调整也会优先围绕现有工程使用体验展开。

## 文档导航

- 使用说明与 FAQ：当前 `README.md`
- 源码与开发说明：[`docs/source_code.md`](docs/source_code.md)
- 目录结构说明：[`docs/project_structure.md`](docs/project_structure.md)
...
├─ bds/                核心通信与波形模块
├─ docs/               开发说明与目录说明
├─ images/             README 配图与演示资源
├─ scripts/            构建脚本
├─ rtt_t2.py           主程序入口
├─ config_manager.py   配置与日志目录管理
├─ text_searcher.py    日志文本搜索
├─ rtt_diag.py         RTT 诊断脚本
...
项目保留了根目录的 `build.bat` 作为兼容入口，实际打包脚本已收拢到 `scripts/build.bat`。
...
#### <保存全部数据>功能该怎么用？
点击**保存全部数据**按钮完成了两件事。一个是将rtt_t2的数据区内的log全部保存到txt文本，另一个是打开这个txt文本。如果你将**txt文本**的**默认打开程序**（通过windows系统）设置为**notepad**或者**其他你喜欢的编辑器**，那么你可以在一个强大的编辑器中搜索、编辑log，这比rtt_t2自带的文本搜索强大的多。
