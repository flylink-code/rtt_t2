...
- **终端输入**：终端模式在画面里直接输入（Xshell / MobaXterm 风格），支持中文输入法、UTF-8、本地回显
- **日志发送栏**：日志模式保留底部命令栏（Enter 发送、↑↓ 历史、可选多行）
...
| **终端模式** | 在终端画面里直接输入（与 Xshell / MobaXterm 相同）：中文输入法、本地回显、方向键 / Tab / Ctrl+C 发给设备 |

**终端模式**在主显示区直接输入，不使用底部命令栏。设备不回显时可在右键菜单打开 **本地回显**。日志模式仍用底部命令栏发送 ASC / HEX。勾选 **多行** 可切到多行发送框。
...
| 终端模式 | 终端内直接输入（IME、本地回显、Tab / Ctrl+C / 方向键） |
| 波形 | 最多 3 条曲线，拖拽、区域统计、CSV 导出 |
| 发送 | 日志模式底部命令栏 Enter 发送、↑↓ 历史、中文输入法；可选多行 / ASC / HEX |
